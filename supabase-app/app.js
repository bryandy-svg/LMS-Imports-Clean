...
const tableMap = {
  ...
  assets: { table: "assets", key: "asset_tag", title: "Fleet & Equipment", sub: "Track vehicles, heavy equipment, readings, ownership, locations, photos, QR codes, and parent/child attachments.", heads: ["photo_url", "qr_update_url", "old_qr_code", "asset_tag", "type", "general_type", "name", "parent_asset_tag", "relationship_type", "compatible_with", "make", "model", "color", "vin_serial", "plate", "location", "last_update_date", "scanned_date", "odometer", "engine_hours", "assigned_operator", "status"], labels: ["Photo", "New QR", "Old QR Code", "Asset", "Type", "General Type", "Name", "Parent Asset", "Relationship", "Compatible With", "Make", "Model", "Color", "VIN / Serial", "Plate", "Location", "Last Update", "Scanned Date", "Odometer", "Hours", "Operator", "Status"] },
...
function mechanicVisibleWorkOrders(mechanic) {
  ...
  const rows = currentRows.filter((wo) => {
    ...
    const assigned = (wo._issues || []).some((i) => !i.assigned_mechanic || String(i.assigned_mechanic || "").toLowerCase() === name);
    const hasLabor = (wo._labor || []).some((l) => String(l.mechanic || "").toLowerCase() === name);
    return assigned || hasLabor || !name;
  });
  if (active && mechanicWorkOrderOpenForPortal(active) && !rows.some((wo) => wo.id === active.id)) rows.unshift(active);
  return rows.length ? rows : currentRows.filter(mechanicWorkOrderOpenForPortal);
}
...
function assetTableHtml(rows) {
  const heads = ["Photo", "New QR", "Old QR Code", "Asset", "Type", "General Type", "Parent Asset", "Relationship", "Compatible With", "Make / Model", "Color", "VIN / Serial", "Plate", "Location", "Last Update", "Scanned Date", "Reading", "Operator", "Status", ""];
...
function assetRowHtml(asset) {
  ...
  return `<tr>
    ...
    <td>${esc(asset.last_update_date || "")}</td>
...
async function openAssetModal(asset = null) {
  ...
  $("modalBody").innerHTML = `
    <div class="form-grid">
      ...
      ${productInput("Old QR Code", "old_qr_code", asset?.old_qr_code || "")}
      ...
      ${productInput("Last update date", "last_update_date", asset?.last_update_date || "", "date")}
...
function exportAssetsWithQrCsv() {
  const heads = ["Photo URL", "New QR Update URL", "Old QR Code", "Asset", "Type", "General Type", "Name", "Parent Asset", "Relationship", "Compatible With", "Make", "Model", "Color", "VIN / Serial", "Plate", "Location", "Last Update Date", "Scanned Date", "Odometer", "Engine Hours", "Operator", "Status"];
  const rows = [heads, ...currentRows.map((a) => [a.photo_url || "", a.qr_update_url || assetQrUrl(a), a.old_qr_code || "", a.asset_tag, a.type, a.general_type || "", a.name, a.parent_asset_tag || "", a.relationship_type || "", a.compatible_with || "", a.make, a.model, a.color, a.vin_serial, a.plate, a.location, a.last_update_date || "", a.scanned_date, a.odometer, a.engine_hours, a.assigned_operator, a.status])];
...
async function importLocalState(local) {
  ...
  const assets = (local.assets || []).map((a) => ({
    ...
    old_qr_code: a.oldQrCode || null,
    last_update_date: a.lastUpdateDate || null,
