...
-- Signed-in users must also be able to read and update the shared inventory
-- and request registers according to their assigned access checkboxes.
grant select, insert, update, delete on table public.inventory_items to authenticated;
grant select, insert, update, delete on table public.inventory_requests to authenticated;

drop policy if exists "Assigned users read inventory" on public.inventory_items;
drop policy if exists "Assigned users insert inventory" on public.inventory_items;
drop policy if exists "Assigned users update inventory" on public.inventory_items;
drop policy if exists "Assigned users delete inventory" on public.inventory_items;
drop policy if exists "Assigned users read requests" on public.inventory_requests;
drop policy if exists "Assigned users insert requests" on public.inventory_requests;
drop policy if exists "Assigned users update requests" on public.inventory_requests;
drop policy if exists "Assigned users delete requests" on public.inventory_requests;

create policy "Assigned users read inventory" on public.inventory_items
for select to authenticated using (
  lower(coalesce(auth.jwt()->>'email','')) = 'bryan.dy@lmsfm.com'
  or coalesce(auth.jwt()->'app_metadata'->'access','[]'::jsonb) ?| array['inventory','requests','manage_users']
);
create policy "Assigned users insert inventory" on public.inventory_items
for insert to authenticated with check (
  lower(coalesce(auth.jwt()->>'email','')) = 'bryan.dy@lmsfm.com'
  or coalesce(auth.jwt()->'app_metadata'->'access','[]'::jsonb) ?| array['inventory','requests','manage_users']
);
create policy "Assigned users update inventory" on public.inventory_items
...
create policy "Assigned users delete inventory" on public.inventory_items
for delete to authenticated using (
  lower(coalesce(auth.jwt()->>'email','')) = 'bryan.dy@lmsfm.com'
  or coalesce(auth.jwt()->'app_metadata'->'access','[]'::jsonb) ?| array['inventory','manage_users']
);

create policy "Assigned users read requests" on public.inventory_requests
for select to authenticated using (
  lower(coalesce(auth.jwt()->>'email','')) = 'bryan.dy@lmsfm.com'
  or coalesce(auth.jwt()->'app_metadata'->'access','[]'::jsonb) ?| array['requests','manage_users']
);
create policy "Assigned users insert requests" on public.inventory_requests
for insert to authenticated with check (
  lower(coalesce(auth.jwt()->>'email','')) = 'bryan.dy@lmsfm.com'
  or coalesce(auth.jwt()->'app_metadata'->'access','[]'::jsonb) ?| array['requests','manage_users']
);
create policy "Assigned users update requests" on public.inventory_requests
for update to authenticated using (
  lower(coalesce(auth.jwt()->>'email','')) = 'bryan.dy@lmsfm.com'
  or coalesce(auth.jwt()->'app_metadata'->'access','[]'::jsonb) ?| array['requests','manage_users']
) with check (
...
create policy "Assigned users delete requests" on public.inventory_requests
for delete to authenticated using (
  lower(coalesce(auth.jwt()->>'email','')) = 'bryan.dy@lmsfm.com'
  or coalesce(auth.jwt()->'app_metadata'->'access','[]'::jsonb) ?| array['requests','manage_users']
);
...
create table if not exists public.greenwaste_tickets (
  ...
  first_scale_at timestamptz,
  second_scale_at timestamptz,
  jobsite text,
  ...
  billing_invoice_reference text,
...
alter table public.greenwaste_tickets add column if not exists status text not null default 'Finalized';
alter table public.greenwaste_tickets add column if not exists first_scale_at timestamptz;
alter table public.greenwaste_tickets add column if not exists second_scale_at timestamptz;
...
alter table public.greenwaste_tickets add column if not exists billing_invoice_reference text;
...
create index if not exists greenwaste_tickets_status_idx on public.greenwaste_tickets (status);
create index if not exists greenwaste_tickets_billing_invoice_reference_idx on public.greenwaste_tickets (billing_invoice_reference);
create index if not exists user_access_created_by_idx on public.user_access (created_by);

create table if not exists public.greenwaste_customers (
  id uuid primary key,
...
create table if not exists public.greenwaste_rates (
  id uuid primary key,
...
create index if not exists greenwaste_customers_name_idx on public.greenwaste_customers (customer_name);
create index if not exists greenwaste_customers_created_by_idx on public.greenwaste_customers (created_by);
create index if not exists greenwaste_rates_name_idx on public.greenwaste_rates (rate_name);

alter table public.user_access enable row level security;
alter table public.greenwaste_tickets enable row level security;
alter table public.greenwaste_customers enable row level security;
alter table public.greenwaste_rates enable row level security;

revoke all on table public.user_access from anon;
revoke all on table public.greenwaste_tickets from anon;
revoke all on table public.greenwaste_customers from anon;
revoke all on table public.greenwaste_rates from anon;
...
grant select, insert, update, delete on table public.greenwaste_customers to authenticated;
grant select, insert, update, delete on table public.greenwaste_rates to authenticated;
...
-- Use the current assignment row as well as JWT metadata. This makes newly
-- assigned Request access effective immediately without recreating the login.
alter policy "Assigned users read requests" on public.inventory_requests
using (
  lower(coalesce(auth.jwt()->>'email','')) = 'bryan.dy@lmsfm.com'
  or coalesce(auth.jwt()->'app_metadata'->'access','[]'::jsonb) ?| array['requests','manage_users']
  or exists (
...
alter policy "Assigned users insert requests" on public.inventory_requests
with check (
  lower(coalesce(auth.jwt()->>'email','')) = 'bryan.dy@lmsfm.com'
  or coalesce(auth.jwt()->'app_metadata'->'access','[]'::jsonb) ?| array['requests','manage_users']
  or exists (select 1 from public.user_access ua where ua.user_id = auth.uid() and ua.access && array['requests','manage_users']::text[])
);
alter policy "Assigned users update requests" on public.inventory_requests
using (
  lower(coalesce(auth.jwt()->>'email','')) = 'bryan.dy@lmsfm.com'
  or coalesce(auth.jwt()->'app_metadata'->'access','[]'::jsonb) ?| array['requests','manage_users']
  or exists (select 1 from public.user_access ua where ua.user_id = auth.uid() and ua.access && array['requests','manage_users']::text[])
...
alter policy "Assigned users delete requests" on public.inventory_requests
using (
  lower(coalesce(auth.jwt()->>'email','')) = 'bryan.dy@lmsfm.com'
  or coalesce(auth.jwt()->'app_metadata'->'access','[]'::jsonb) ?| array['requests','manage_users']
  or exists (select 1 from public.user_access ua where ua.user_id = auth.uid() and ua.access && array['requests','manage_users']::text[])
...
drop policy if exists "Greenwaste users read customers" on public.greenwaste_customers;
create policy "Greenwaste users read customers" on public.greenwaste_customers
for select to authenticated
using (coalesce((select auth.jwt()) -> 'app_metadata' -> 'access', '[]'::jsonb) ? 'greenwaste');

drop policy if exists "Greenwaste users create customers" on public.greenwaste_customers;
create policy "Greenwaste users create customers" on public.greenwaste_customers
for insert to authenticated
with check (
  coalesce((select auth.jwt()) -> 'app_metadata' -> 'access', '[]'::jsonb) ? 'greenwaste'
  and created_by = (select auth.uid())
...
drop policy if exists "Greenwaste users update customers" on public.greenwaste_customers;
create policy "Greenwaste users update customers" on public.greenwaste_customers
for update to authenticated
using (coalesce((select auth.jwt()) -> 'app_metadata' -> 'access', '[]'::jsonb) ? 'greenwaste')
with check (coalesce((select auth.jwt()) -> 'app_metadata' -> 'access', '[]'::jsonb) ? 'greenwaste');

drop policy if exists "Greenwaste users delete customers" on public.greenwaste_customers;
create policy "Greenwaste users delete customers" on public.greenwaste_customers
for delete to authenticated
using (coalesce((select auth.jwt()) -> 'app_metadata' -> 'access', '[]'::jsonb) ? 'greenwaste');

drop policy if exists "Greenwaste users read rates" on public.greenwaste_rates;
create policy "Greenwaste users read rates" on public.greenwaste_rates for select to authenticated
using (coalesce((select auth.jwt()) -> 'app_metadata' -> 'access', '[]'::jsonb) ? 'greenwaste');
drop policy if exists "Greenwaste users create rates" on public.greenwaste_rates;
create policy "Greenwaste users create rates" on public.greenwaste_rates for insert to authenticated
with check (coalesce((select auth.jwt()) -> 'app_metadata' -> 'access', '[]'::jsonb) ? 'greenwaste' and created_by = (select auth.uid()));
drop policy if exists "Greenwaste users update rates" on public.greenwaste_rates;
create policy "Greenwaste users update rates" on public.greenwaste_rates for update to authenticated
using (coalesce((select auth.jwt()) -> 'app_metadata' -> 'access', '[]'::jsonb) ? 'greenwaste')
with check (coalesce((select auth.jwt()) -> 'app_metadata' -> 'access', '[]'::jsonb) ? 'greenwaste');
drop policy if exists "Greenwaste users delete rates" on public.greenwaste_rates;
create policy "Greenwaste users delete rates" on public.greenwaste_rates for delete to authenticated
using (coalesce((select auth.jwt()) -> 'app_metadata' -> 'access', '[]'::jsonb) ? 'greenwaste');
